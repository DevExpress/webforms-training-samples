...
namespace sample_3_10 {
  public partial class VenuesRooms : System.Web.UI.Page {
    ...
    const string STR_UploadedImageBytes = "VENUE_IMAGE_UPLOAD";
    string[] imageTypes = new string[] { "image/jpg", "image/jpeg", "image/png" };

    protected void uploadControl_FileUploadComplete(object sender, FileUploadCompleteEventArgs e) {
      if (imageTypes.Any(it => String.Equals(it, e.UploadedFile.ContentType, StringComparison.InvariantCultureIgnoreCase))) {
        // Note: consider using a better validity check in real applications
        Session[STR_UploadedImageBytes] = e.UploadedFile.FileBytes;
      }
      else {
        e.IsValid = false;
      }
    ...
    protected void callbackPanel_Callback(object sender, CallbackEventArgsBase e) {
      if (sender is ASPxCallbackPanel panel &&
        panel.FindControl("previewImage") is ASPxBinaryImage previewImage)
        previewImage.ContentBytes = (byte[])Session[STR_UploadedImageBytes];
    }

    protected void venueGrid_RowUpdating(object sender, DevExpress.Web.Data.ASPxDataUpdatingEventArgs e) {
      if (Session[STR_UploadedImageBytes] != null)
        e.NewValues["PhotoBytes"] = Session[STR_UploadedImageBytes];
    }

    protected void venueGrid_RowInserting(object sender, DevExpress.Web.Data.ASPxDataInsertingEventArgs e) {
      if (Session[STR_UploadedImageBytes] != null)
        e.NewValues["PhotoBytes"] = Session[STR_UploadedImageBytes];
    }
  }
